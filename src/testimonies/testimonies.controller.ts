...
export class TestimoniesController {
  ...
  @ApiResponse({
    status: 500,
    description: 'Server error while creating testimony.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Filter by category using categoryId or categorySlug (slug takes precedence if both provided). Includes pagination and search filtering.',
  })
  @ApiResponse({
    status: 200,
    description:
      'List of testimonies (each includes category) with pagination metadata.',
  })
  @ApiResponse({
    status: 404,
    description: 'Category not found (when using categorySlug).',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns only approved testimonies. Filter by categoryId or categorySlug. Includes pagination and search filtering.',
  })
  @ApiResponse({
    status: 200,
    description: 'List of approved testimonies with pagination metadata.',
  })
  @ApiResponse({
    status: 404,
    description: 'Category not found (when using categorySlug).',
  ...
  @ApiOperation({
    ...
    description:
      'Returns only rejected testimonies. Filter by categoryId or categorySlug. Includes pagination and search filtering.',
  })
  @ApiResponse({
    status: 200,
    description: 'List of rejected testimonies with pagination metadata.',
  })
  @ApiResponse({
    status: 404,
    description: 'Category not found (when using categorySlug).',
  ...
  @ApiOperation({
    ...
    description:
      'Returns only testimonies awaiting review. Filter by categoryId or categorySlug. Includes pagination and search filtering.',
  })
  @ApiResponse({
    status: 200,
    description: 'List of pending testimonies with pagination metadata.',
  })
  @ApiResponse({
    status: 404,
    description: 'Category not found (when using categorySlug).',
  ...
  findOne(
    ...
    return this.testimoniesService.findOneAndIncrementViews(
      id,
      session.viewedTestimonies,
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      'Requires admin authentication. Set status to APPROVED or REJECTED.',
  })
  ...
  update(
    ...
    return this.testimoniesService.update(
      id,
      updateTestimonyDto,
      req.user.email,
    );
